...
async function main() {
    ...
    const factory = await ethers.getContractFactory("XLARSCValveFactory");
    ...
    let encodedData = rc.events?.filter((x) => {return x.event == "RSCValveCreated"});
    const decoder = new ethers.utils.AbiCoder();
    let decodedData = decoder.decode(["address","address","address[]","uint256","bool","bool","uint256","bytes32"], encodedData[0].data);
